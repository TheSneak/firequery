...
FBL.ns(function() {
    with(FBL) {
        ...
        var patchJQuery = function(jQuery, context) {
            ...
            jQuery.__FireQueryShared.data_originalReplacedByFireQuery = function(elem, name, data, pvt) {
                var res = jQuery.__FireQueryShared.oldData.apply(this,Array.prototype.slice.apply(arguments));
                return {
                    res: res,
                    __exposedProps__ : {
                        res: "rw"
            ...
            jQuery.data = function(elem, name, data, pvt) {
                var originalDataImplementation = jQuery_data;
                ...
                if (writing) {
                        ...
                        if(forceInternals)
                                var snapshot = jQuery._data(elem, undefined, undefined);
                        else
                                var snapshot =  originalDataImplementation(elem, undefined, undefined,forceInternals);
                    var oldData = jQuery.__FireQueryShared.extend(true, {}, snapshot); // need to do a deep copy of the whole structure
                }
                var res = originalDataImplementation(elem, name, data, pvt);
                if (writing) {
                    try {
                        //var newData = originalDataImplementation.apply(this, [elem, undefined, undefined, forceInternals]).res;
						
                        if(forceInternals)
                           var newData = jQuery._data(elem, undefined, undefined);
                        else
                            var newData = originalDataImplementation(elem, undefined, undefined,pvt);
            ...
            jQuery.removeData = function(elem, name) {
				...
				if(forceInternals)
					...
					var snapshot = originalDataImplementation(elem, undefined, undefined);
                var oldData = jQuery.__FireQueryShared.extend(true, {}, snapshot); // need to do a deep copy of the whole structure
                var res = jQuery_removeData(elem, name);
                try {
                    //var newData = originalDataImplementation.apply(this, [elem, undefined, undefined, forceInternals]).res;
					if(forceInternals)
						var newData = jQuery._data(elem, undefined, undefined);
					else
						var newData = originalDataImplementation(elem, undefined, undefined);
